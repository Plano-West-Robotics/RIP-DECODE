...
@Autonomous
public class RedFarBNFStateMachineAuto extends BaseAuto
{
...
//        }
    ...
    */
    ...
    public void createPaths()
    {
        ...
        headings[pathCount] = new double[] {AutonConstants.FAR_RED_BNF_INTERMEDIATE.getHeading()};
        poses[pathCount++] = new Pose[]
            {
    ...
    public State[] buildMachine()
    {
        ...
        states[1] = new BaseState("AT_PRELOAD_SCORE")
            ...
            .addTransition(new Transition(() -> pathTimer.getElapsedTimeSeconds() > AutonConstants.PRELOAD_SCORE_TIME, "TO_INTERMEDIATE1"));

        states[2] = new BaseState("TO_INTERMEDIATE1")
            .setExit(() -> {
                follower.followPath(paths[2]);
            })
            .addTransition(new Transition(() -> !follower.isBusy(), "TO_LINEUP1"));

        states[3] = new BaseState("TO_LINEUP1")
            .setExit(() -> {
                follower.followPath(paths[3]);
            })
            .addTransition(new Transition(() -> !follower.isBusy(), "TO_INTAKE1"));

        states[4] = new BaseState("TO_INTAKE1")
            .setExit(() -> {
        ...
        states[5] = new BaseState("TO_SCORE1")
            .setDuring(() -> {
                intake.stop();
                intake.stopTransfer();
        ...
        states[6] = new BaseState("AT_SCORE1")
            .setEntry(() -> {
                pathTimer.resetTimer();
            })
            .setDuring(this::shootArtifacts)
