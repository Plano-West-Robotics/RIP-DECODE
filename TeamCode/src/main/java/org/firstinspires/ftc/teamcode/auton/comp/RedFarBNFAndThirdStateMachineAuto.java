...
@Autonomous
public class RedFarBNFAndThirdStateMachineAuto extends BaseAuto
{
...
//        }
    ...
    */
    ...
    public void createPaths()
    {
        ...
        headings[pathCount] = new double[] {AutonConstants.FAR_RED_START.getHeading(), AutonConstants.FAR_RED_SCORE.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_START, AutonConstants.FAR_RED_SCORE
            };

        headings[pathCount] = new double[] {AutonConstants.FAR_RED_SCORE.getHeading(), AutonConstants.FAR_RED_LINEUP1.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_SCORE, AutonConstants.FAR_RED_LINEUP1
            };

        headings[pathCount] = new double[] {AutonConstants.FAR_RED_LINEUP1.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_LINEUP1, AutonConstants.FAR_RED_INTAKE1
            };

        headings[pathCount] = new double[] {AutonConstants.FAR_RED_INTAKE1.getHeading(), AutonConstants.FAR_RED_SCORE.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_INTAKE1, AutonConstants.FAR_RED_SCORE
            };

        headings[pathCount] = new double[] {AutonConstants.FAR_RED_SCORE.getHeading(), AutonConstants.FAR_RED_BNF_INTERMEDIATE.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_SCORE, AutonConstants.FAR_RED_BNF_INTERMEDIATE
            };

        headings[pathCount] = new double[] {AutonConstants.FAR_RED_BNF_INTERMEDIATE.getHeading(), AutonConstants.FAR_RED_BNF_LINEUP.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_BNF_INTERMEDIATE, AutonConstants.FAR_RED_BNF_LINEUP
            };

        headings[pathCount] = new double[] {AutonConstants.FAR_RED_BNF_LINEUP.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_BNF_LINEUP, AutonConstants.FAR_RED_BNF_INTAKE
            };

        headings[pathCount] = new double[] {AutonConstants.FAR_RED_BNF_INTAKE.getHeading(), AutonConstants.FAR_RED_SCORE.getHeading()};
        poses[pathCount++] = new Pose[]
            {
                AutonConstants.FAR_RED_BNF_INTAKE, AutonConstants.FAR_RED_SCORE
            };
        ...
        */
        ...
        paths[6].setVelocityConstraint(AutonConstants.INTAKE_1_VEL_CONSTRAINT);
    }
    ...
    public State[] buildMachine()
    {
        ...
        states[1] = new BaseState("AT_PRELOAD_SCORE")
            ...
            .addTransition(new Transition(() -> pathTimer.getElapsedTimeSeconds() > AutonConstants.PRELOAD_SCORE_TIME, "TO_SPIKE_LINEUP"));

        states[2] = new BaseState("TO_SPIKE_LINEUP")
            .setExit(() -> {
                follower.followPath(paths[2]);
            })
            .addTransition(new Transition(() -> !follower.isBusy(), "TO_SPIKE_INTAKE"));

        states[3] = new BaseState("TO_SPIKE_INTAKE")
            .setExit(() -> {
                follower.followPath(paths[3]);
                pathTimer.resetTimer();
            })
            .addTransition(new Transition(() -> !follower.isBusy(), "TO_SCORE_FROM_SPIKE"));

        states[4] = new BaseState("TO_SCORE_FROM_SPIKE")
            .setDuring(() -> {
                intake.stop();
                intake.stopTransfer();
        ...
        states[5] = new BaseState("TO_INTERMEDIATE1")
            .setExit(() -> {
                follower.followPath(paths[5]);
            })
            .addTransition(new Transition(() -> !follower.isBusy(), "TO_LINEUP1"));

        states[6] = new BaseState("TO_LINEUP1")
            .setExit(() -> {
                follower.followPath(paths[6]);
            })
            .addTransition(new Transition(() -> !follower.isBusy(), "TO_INTAKE1"));

        states[7] = new BaseState("TO_INTAKE1")
            .setExit(() -> {
        ...
        states[8] = new BaseState("TO_SCORE1")
            .setDuring(() -> {
                intake.stop();
                intake.stopTransfer();
                outtake.setVelocity(Outtake.MANUAL_ANGULAR_RATE);
        ...
        states[9] = new BaseState("AT_SCORE1")
            .setEntry(() -> {
                pathTimer.resetTimer();
            })
            .setDuring(this::shootArtifacts)
            ...
            .addTransition(new Transition(() -> pathTimer.getElapsedTimeSeconds() > AutonConstants.FIRST_THREE_SCORE_TIME, "TO_INTERMEDIATE1"));
